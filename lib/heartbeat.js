 ...
 *   onHealthy    () => void                    (a valid payload landed: the
 *                                               account works again, so this is
 *                                               where a quarantine gets lifted)
 */
export function createConfigHeartbeat(config, log = null, deps = {}) {
  ...
  const onHealthy = deps.onHealthy || (() => {});
  ...
  // The payload was valid, so the account answers again. This is the cheap
  // ban-lift probe the header comment promises: a hook that throws is contained
  // (the poll's own job is the catalog), and a hook with nothing to do returns
  // silently — the governor only logs when a quarantine actually lifts.
  function reportHealthy() {
    try { onHealthy(); }
    catch (err) { debug(`heartbeat: onHealthy hook failed (${err.message || err}) — ignored`); }
  }
