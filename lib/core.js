...
export function readRuntimeModels(config) {
  for (const candidate of config.RUNTIME_CANDIDATES) {
    try {
      ...
      if (models.length > 0) {
        return { models, source: candidate };
      }
...
// Keep this snapshot helper for startup display. Request handlers should call
// getModelCatalog so runtime model changes are visible without a proxy restart.
...
// OpenAI variant: flattens text-object arrays before forwarding.
