...
export function loadConfig({ defaultPort, format = "openai" }) {
  ...
  const MAX_MESSAGES   = parseInt(process.env.MAX_MESSAGES || "128", 10) || 128; // entity / message limit
  ...
  return {
    PORT, PROXY_KEY, LOG_LEVEL, MAX_BODY_BYTES, RATE_LIMIT, MAX_MESSAGES, PREFER_LOCAL,
...
export function validateChatPayload(body, maxMessages = 128) {
  const MAX_MESSAGES = maxMessages || 128;
          ...
          if (msg.event === 'connect.challenge') {
            socket.write(encodeWsFrame(JSON.stringify({
              ...
              params: {
                ...
                caps: ['tool_events'], commands: [], permissions: {}, auth: { token }, locale: 'en', userAgent: 'glmproxy/2.0.0'
              }
            })));
