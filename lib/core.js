...
// upstream calls, error classification, loggers, server
...
import { decodeBuffer, decodeStream } from "./decode.js";
import { createConfigHeartbeat } from "./heartbeat.js";
import { createShapeLayer, PINNED_BANNER } from "./shape.js";
...
// Retry defaults for a capacity throttle (see `callUpstreamWithRetry`). Two
// attempts past the first, four seconds apart and growing 1.5× — long enough
// for the upstream's own "try again shortly" to be true, short enough that a
// harness with a 30 s patience still gets an answer. Exported so the taxonomy
// suite can pin them instead of trusting prose.
export const DEFAULT_THROTTLE_RETRIES = 2;
export const DEFAULT_THROTTLE_RETRY_MS = 4000;
...
export function loadConfig({ format = "openai", defaultPort = DEFAULT_PORTS[format] } = {}) {
  ...
  // PREFER_LOCAL used to skip the cloud entirely and drive AutoClaw's own
  // desktop agent over a local WebSocket instead. That route is deleted (see the
  // tombstone above the upstream caller). The variable is still read, but only
  // in order to say so out loud: an env var that silently stops doing anything
  // is worse than one that reports it was removed. The old behaviour was also a
  // hard outage when the desktop app was down — it skipped the cloud and then
  // never fell back to it (502, zero upstream hits).
  if (process.env.PREFER_LOCAL === "1") {
    console.warn(
      "PREFER_LOCAL is ignored: the local AutoClaw WebSocket agent fallback was removed. " +
      "Requests always run against the cloud now, and a throttle is retried in-request " +
      "instead of being replayed as an agentic session inside the desktop app."
    );
  }
  ...
  // Request-shape parity (Requirement 4).
  //
  // Prompt envelope: a foreign harness sends a 30 KB system prompt the app never
  // would. Over this envelope the proxy drops the middle of the *system* message
  // and marks the cut, preserving head and tail. 0/unset = off, which is the
  // default: compaction changes the prompt a client asked for, so it is opt-in
  // per the layer-isolation rule, not something we do behind the operator's back.
  const PROMPT_ENVELOPE_KB = (() => {
    const raw = process.env.PROMPT_ENVELOPE_KB;
    if (raw === undefined || raw === "") return 0;
    const n = parseInt(raw, 10);
    return Number.isFinite(n) && n > 0 ? n : 0;
  })();

  // Structural fields to drop from tool entries. Empty by default - the audit
  // saw tools pass through upstream unchanged, so the honest default is identity
  // and a field is only listed once there is evidence upstream rejects it.
  const TOOL_SHAPE_BLOCKLIST = (process.env.TOOL_SHAPE_BLOCKLIST || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);

  // Where the installed app's prompt builder lives. Same discovery stance as the
  // runtime file: an explicit override first, then the platform-default install
  // roots. Nothing here is required - a miss just falls through the chain.
  const GATEWAY_DIST_CANDIDATES = (() => {
    // An explicit GLMP_GATEWAY_DIST is an OVERRIDE, not a hint: it replaces the
    // platform roots instead of being tried ahead of them. That is the same
    // stance as GLMP_IDENTITY_VERSION, and it is what makes this testable - a
    // fake HOME cannot hide a real AutoClaw install, because these roots come
    // from ProgramFiles/LOCALAPPDATA rather than from the home directory. Point
    // it at one dist to pin discovery, or at nothing to exercise the lower rungs.
    const explicit = (process.env.GLMP_GATEWAY_DIST || "").trim();
    if (explicit) return [explicit];

    const out = [];
    const push = (p) => { if (p && !out.includes(p)) out.push(p); };
    const rel = ["resources", "gateway", "openclaw", "dist"];
    const roots = [];
    if (process.env.AUTOCLAW_INSTALL_DIR) roots.push(process.env.AUTOCLAW_INSTALL_DIR);
    if (process.platform === "win32") {
      if (process.env.ProgramFiles) roots.push(path.join(process.env.ProgramFiles, "AutoClaw"));
      if (process.env.LOCALAPPDATA) {
        roots.push(path.join(process.env.LOCALAPPDATA, "Programs", "AutoClaw"));
        roots.push(path.join(process.env.LOCALAPPDATA, "AutoClaw"));
      }
    } else if (process.platform === "darwin") {
      roots.push("/Applications/AutoClaw.app/Contents/Resources");
    } else {
      roots.push("/opt/AutoClaw");
    }
  ...
  // Retrying a capacity throttle in-request instead of surfacing it. The
  // desktop-agent fallback that used to absorb a throttle is gone, and a harness
  // that receives a hard 429 aborts its turn — so a short bounded wait is the
  // difference between "slower answer" and "failed request". The base wait is
  // jittered (±20%) and grows 1.5× per attempt so the retries do not look like a
  // hammer. 0 retries restores the raw classified error.
  const THROTTLE_RETRIES = (() => {
    const raw = process.env.GLMP_THROTTLE_RETRIES;
    if (raw === undefined || raw === "") return DEFAULT_THROTTLE_RETRIES;
    const n = parseInt(raw, 10);
    return Number.isFinite(n) && n >= 0 ? n : DEFAULT_THROTTLE_RETRIES;
  })();
  const THROTTLE_RETRY_MS = (() => {
    const raw = process.env.GLMP_THROTTLE_RETRY_MS;
    if (raw === undefined || raw === "") return DEFAULT_THROTTLE_RETRY_MS;
    const n = parseInt(raw, 10);
    return Number.isFinite(n) && n >= 0 ? n : DEFAULT_THROTTLE_RETRY_MS;
  })();
  ...
  return {
    PORT, HOST, PROXY_KEY, LOG_LEVEL, MAX_BODY_BYTES, RATE_LIMIT, MAX_MESSAGES,
    ...
    HEARTBEAT_INTERVAL_MS, PROMPT_ENVELOPE_KB, TOOL_SHAPE_BLOCKLIST, GATEWAY_DIST_CANDIDATES,
    UPSTREAM_TIMEOUT_MS, THROTTLE_RETRIES, THROTTLE_RETRY_MS,
...
export function getConfigHeartbeat(config, log = null, deps = {}) {
  ...
  if (!heartbeat) {
    heartbeat = createConfigHeartbeat(config, log, {
      ...
      // Task 7a: the poll that just proved the account works is also the moment
      // a quarantine should end. Without this hook `liftQuarantine()` is dead
      // code and a false-positive ban (the stale-identity class, which does
      // happen) is permanent until the operator deletes the state file by hand.
      onHealthy: deps.onHealthy || (() => {
        getPacingGovernor(config, log, { getToken: deps.getToken }).liftQuarantine();
      }),
    });
...
// ---------------------------------------------------------------------------
// Shape layer (Requirement 4).
//
// Memoized per configured envelope + gateway-dist set, so both entrypoints share
// ONE resolved banner and one snapshot. `AUTOCLAW_SYSTEM_BANNER` and
// `injectSystemBanner` survive below as thin wrappers so the existing call sites
// (and the experiments probe) migrate by changing nothing.
// ---------------------------------------------------------------------------
const _shapeLayers = new Map();

export function getShapeLayer(config, log = null) {
  // Everything the layer bakes in at construction belongs in the key, or a
  // second config silently reuses the first one's resolved banner and settings:
  //   * the state dir - the banner snapshot lives there, so two state dirs must
  //     not share one resolved banner;
  //   * the envelope and the blocklist - both are captured into the layer.
  const key = [
    config?.PROMPT_ENVELOPE_KB || 0,
    (config?.GATEWAY_DIST_CANDIDATES || []).join("|"),
    config?.STATE_DIR || "",
    (config?.TOOL_SHAPE_BLOCKLIST || []).join(","),
  ].join("|");
  let layer = _shapeLayers.get(key);
  if (!layer) {
    layer = createShapeLayer(config, log);
    _shapeLayers.set(key, layer);
  }
  return layer;
}

// The banner the proxy will prepend, resolved through discovery → snapshot →
// env override → pinned. Kept as a function rather than a constant, because a
// constant is exactly what rotted before.
export function getSystemBanner(config) {
  return getShapeLayer(config).getBanner().text;
}

export function closeShapeLayers() {
  for (const layer of _shapeLayers.values()) {
    try { layer.stop(); } catch (_) { /* shutdown must never throw */ }
  }
  _shapeLayers.clear();
}

// Hot-reload the banner when the installed app's prompt builder changes (the
// identity layer's startWatch precedent) - an app update must not need a proxy
// restart to be picked up.
export function startShapeWatch(config, log = null) {
  const layer = getShapeLayer(config, log);
  layer.startWatch();
  return layer;
}
...
export function installShutdownHooks(config = null, log = null) {
  const release = () => {
    ...
    try { closeShapeLayers(); } catch (_) { /* never block exit */ }
...
// account works again shortly, and the message says so. It is classified as a
// rate limit on purpose: that mapping is what lets `callUpstreamWithRetry`
// recognise it as worth a short wait (`isRetryableThrottle`), and it hands a
// client that still ends up seeing the error a `Retry-After` it can act on.
//
// A live session measured on 2026-09-21 showed what the old alternative cost:
// every 810002 event fell into the local AutoClaw WS agent, which re-issued the
// *same* cloud call and came back with the same 403 (`FailoverError: HTTP 403 …
// /autoclaw-proxy/proxy/autoclaw/chat/completions`) — 13 of 15 attempts burned
// the full 120 s budget while the harness had already timed out and retried.
...
//
// Note the ban branch no longer mentions a fallback: there is only one route to
// the model, so a 410004 is terminal for the client and permanent for the
// account, full stop.
export function classifyUpstreamError(statusCode, bodyText, modelName) {
  const text   = typeof bodyText === "string" ? bodyText : "";
  const detail = getUpstreamErrorMessage(text);

  // Account bans outrank generic 403s - a ban is terminal for the account,
  // so no amount of retrying can change the verdict.
  ...
  // so the capacity retry (isRetryableThrottle) waits it out in place: the
  // throttle means "try again shortly", and a few seconds later the same call
  // succeeds. The local desktop agent that used to be tried here shared this
  // account and the same endpoint, so it could only fail the same way — one full
  // timeout later. That route is deleted.
...
export function classifyTransportError(err) {
  ...
  // A content-coding we have no decoder for. This must be named and loud: the
  // alternative is handing the client bytes that are still compressed and
  // calling them text/event-stream, which looks like a working response and is
  // the exact silent-corruption case lib/decode.js exists to make impossible.
  if (err?.code === "UPSTREAM_ENCODING_UNSUPPORTED") {
    return {
      status: 502, type: "api_error", code: "upstream_encoding_unsupported", permanent: false,
      message: msg,
    };
  }
  return {
...
// Single shared decision for "is this failed verdict worth another attempt".
//
// This is what replaced the old "engage the local gateway" decision, and it is
// a different question on purpose. The proxy has exactly one route to the model
// now (the cloud), so there is nothing to fall back *to* — the only lever left
// is whether to wait and re-issue the same call.
//
// Only capacity-shaped verdicts qualify. A ban, a quota wall and a bad request
// are all deterministic: another attempt spends time and changes nothing, and a
// retried ban is exactly the account-damaging behaviour the governor exists to
// prevent.
export const RETRYABLE_VERDICTS = new Set([
  "upstream_busy",             // the 810002 free-tier "high demand" throttle
  "rate_limited_by_upstream",  // a plain upstream 429
  "upstream_failure",          // a 5xx from the cloud
]);

export function isRetryableThrottle(cls) {
  return !!cls && cls.permanent !== true && RETRYABLE_VERDICTS.has(cls.code);
}
...
// retry sleep → failure (~30s+). With it, repeats fail
...
// Send a classification produced by classifyUpstreamError/classifyGovernorError
...
//
// Decoding happens at the wire boundary in callUpstream(), but not every read
// goes through it: `fetchRemoteModelConfig` reads the raw model-config GET, and
// a compressed body there would parse to nothing and silently degrade the live
// catalog to heuristics. Decoding here is a no-op on a body whose
// `content-encoding` was already stripped by decodeStream(), so both paths are
// covered by one rule: never hand a caller bytes that are still encoded.
//
export function collectResponse(res) {
  return new Promise((resolve) => {
    ...
    res.on("end", () => {
      ...
      try {
        resolve(decodeBuffer(raw, res.headers?.["content-encoding"]).toString("utf8"));
      } catch (_) {
        // A coding we cannot decode must not be reported as a body we understood
        // - an empty body degrades to status-code classification, which is the
        // same honest answer every other unreadable body gets.
        resolve("");
      }
    });
...
// Cloud call with two kinds of retry, both bounded, both inside the same client
// request so a transient upstream mood never becomes the client's problem.
//
//   (a) the historically flaky 400 "invalid request" hiccup — one retry after a
//       fixed 2 s — but never for a model already confirmed permanently broken.
//   (b) a capacity/throttle verdict (`isRetryableThrottle`): a jittered wait that
//       grows 1.5× per attempt, then the same call again. This is what replaced
//       the desktop-agent fallback. That fallback re-ran the harness's prompt as
//       an agentic session inside AutoClaw — same account, same 403, plus a
//       second writer with write access to this machine — and when it failed it
//       failed as a hard API error, which aborts a harness's turn. A client that
//       aborts on 429 and a client that waits 4 s for the same call to succeed
//       see the same upstream; only one of them keeps working.
//
// Buffers and logs every >=400 body along the way (R1). Returns the terminal
// upstream response plus its buffered error body; success rendering stays at the
// call site so wire formats never leak in here.
const INVALID_REQUEST_RETRY_MS = 2000;

export async function callUpstreamWithRetry(callUpstream, modelId, permanentFailures, log, opts = {}) {
  const throttleRetries = Number.isFinite(opts.throttleRetries) ? Math.max(0, opts.throttleRetries) : DEFAULT_THROTTLE_RETRIES;
  const throttleBaseMs = Number.isFinite(opts.throttleBaseMs) ? Math.max(0, opts.throttleBaseMs) : DEFAULT_THROTTLE_RETRY_MS;
  const sleep = opts.sleep || ((ms) => new Promise((r) => setTimeout(r, ms)));
  const random = opts.random || Math.random;

  let invalidRequestRetried = false;
  ...
  for (;;) {
    ...
    if (res.statusCode === 400 && !invalidRequestRetried &&
        !permanentFailures.get(modelId) && errBody.includes('"invalid request"')) {
      invalidRequestRetried = true;
      log.info("Upstream 400 invalid request - retrying once");
      await sleep(INVALID_REQUEST_RETRY_MS);
      continue;
    }

    const cls = classifyUpstreamError(res.statusCode, errBody, modelId);
    if (throttleAttempt < throttleRetries && isRetryableThrottle(cls) && throttleBaseMs > 0) {
      const wait = Math.round(throttleBaseMs * (0.8 + random() * 0.4) * Math.pow(1.5, throttleAttempt));
      throttleAttempt++;
      log.info(
        `Upstream ${res.statusCode} (${cls.code}) — waiting ${(wait / 1000).toFixed(1)}s, ` +
        `then retrying (${throttleAttempt}/${throttleRetries})`
      );
      await sleep(wait);
      continue;
    }
...
// Kept as an alias so an older harness or a third-party caller that imported the
// previous name still resolves; the behaviour it names is inside the retry loop
// above now.
export const callUpstreamWithInvalidRequestRetry = callUpstreamWithRetry;
...
// ============================================================================
// There used to be a local WebSocket bridge here ("L-route"): on a cloud
// failure the proxy opened AutoClaw's own gateway on 127.0.0.1:18789 as role
// operator with operator.write / operator.admin scopes and tool_events, and
// re-ran the harness's prompt as a full agentic session inside the desktop app.
//
// It is deleted, deliberately. It rarely worked, and when it did the work
// happened inside AutoClaw rather than here - so a throttled or failing cloud
// call could start a second agent with write access to this machine, which in a
// coding session looks exactly like a rogue writer editing the same files. When
// the gateway was unavailable it also misreported the failure: a cloud 500
// surfaced to the client as 502 local_gateway_failed with the gateway's
// ECONNREFUSED message instead, and PREFER_LOCAL=1 skipped the cloud entirely
// and then did not fall back to it (502, zero upstream hits).
//
// A cloud failure is now terminal and classified on its own terms. There is no
// second route to the model, on purpose. tests/pen-test-p9.mjs fails if this
// comes back, and PREFER_LOCAL is a stated no-op rather than a silent one.
// ============================================================================
...
// Two mechanisms open the upstream's system-prompt gate, and the proxy now has
// both:
//   * `X-Harness-Type: zcode` - what the app itself declares, probe-verified to
//     open the gate on its own (see loadConfig's HARNESS_TYPE block), and
//   * this banner inside the system prompt - the belt to that suspenders.
//
// The banner is resolved at runtime by lib/shape.js (installed app's own prompt
// builder → last-good snapshot → env override → this pin), because a compiled-in
// copy of a string the app owns is exactly what rotted before: the pin joined the
// lines with a single "\n" while the app emits a *blank line* before "## Tooling"
// (see PINNED_BANNER in lib/shape.js - the corrected byte shape).
//
// lib/shape.js owns injection in buildSanitizedBody below; injectSystemBanner
// survives as the fallback for when the shape layer is unavailable. Full story
// in ROOT-CAUSE-AND-STUDY.md.
export const AUTOCLAW_SYSTEM_BANNER = process.env.AUTOCLAW_SYSTEM_BANNER || PINNED_BANNER;
...
//
// The shape layer (Requirement 4) is applied here, at the single choke point
// where the client body becomes the upstream body - so banner discovery,
// prompt-envelope compaction and tool filtering all happen once, on the way out,
// and the entrypoint's own logging still sees the untouched client body.
// Wrapped so a parity layer can never take down proxying (Requirement 7.3): on
// any throw we fall back to the legacy pinned injector.
function buildSanitizedBody(openAIBody, upstreamModelId, { config = null, log = null } = {}) {
  const clientTools = Array.isArray(openAIBody.tools) && openAIBody.tools.length > 0 ? openAIBody.tools : null;
  ...
  if (config) {
    try {
      const result = getShapeLayer(config, log).apply(messages, clientTools || []);
      messages = result.messages;
      if (clientTools) tools = result.tools;
      shaped = true;
    } catch (err) {
      try { log?.warn?.(`shape: layer failed (${err.message}) - falling back to the pinned banner`); } catch (_) {}
    }
  }
  if (!shaped) messages = injectSystemBanner(messages);
...
async function callUpstream(config, clientHeaders, getToken, sanitizedBody, log, transport = null) {
  ...
  if (active && typeof active.request === "function") {
    ...
    // Decode at this single seam so EVERY consumer of an upstream body gets
    // decoded bytes: the error-body classifier (`collectResponse` → the ban and
    // throttle detection), the streaming passthrough and the buffered paths. A
    // no-op unless the response actually carries a content-coding.
    return decodeStream(res, { log });
  }
  return decodeStream(await postUpstreamWithRetry({
...
export function callUpstreamOpenAI(config, knownIds, clientHeaders, getToken, body, modelId, log, transport = null) {
  ...
  return callUpstream(config, clientHeaders, getToken, buildSanitizedBody(normalized, upstreamModelId, { config, log }), log, transport);
}
...
// `log` is threaded only for the shape layer's compaction/tool-drop notices.
export function callUpstreamAnthropic(config, clientHeaders, getToken, openAIBody, modelId, transport = null, log = null) {
  return callUpstream(config, clientHeaders, getToken, buildSanitizedBody(openAIBody, modelId, { config, log }), log, transport);
}
