...
export function createUpstreamTransport(config, log = null, identityLayer = null, { isTransient = null } = {}) {
  ...
  // auth, content-*), which wins on collision. `Accept-Encoding` is never set:
  // advertising an encoding we cannot decode would obligate decompression for
  // zero parity gain and would change today's byte path (Requirement 2.8).
