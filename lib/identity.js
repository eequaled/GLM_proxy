...
export const DEFAULT_ACCEPT = "application/json, text/event-stream";

// Re-warn about a pinned identity at most this often (Requirement 1.3).
export const STALE_WARN_INTERVAL_MS = 60 * 60 * 1000;

// Platform token for the derived User-Agent, used only when the runtime entry
// does not itself carry an X-Tm.
...
export function createIdentityLayer(config, log = null, clock = defaultClock) {
  ...
  function resolve() {
    ...
    const userAgent = ovUa
      || discovered?.userAgent
      || (version ? `AutoClaw/${version} (${headers["X-Tm"] || platformToken()})` : null);
