...
import { DEFAULT_ACCEPT_ENCODING } from "./decode.js";
...
// The app inherits undici's automatic `accept-encoding` on its fetch calls, so
// the measured value is undici's own advert rather than something the app sets.
// Read from a runtime entry if a future app version ever publishes one; the
// measured default applies otherwise. The decode layer's supported set is the
// same list, so "advertises only what it can decode" holds by construction.
const ACCEPT_ENCODING_KEYS = ["Accept-Encoding", "accept-encoding"];
...
export function createIdentityLayer(config, log = null, clock = defaultClock) {
    ...
    for (const candidate of config?.RUNTIME_CANDIDATES || []) {
      try {
        ...
        return {
          ...
          acceptEncoding: pickFirst(entry, ACCEPT_ENCODING_KEYS),
          source: "runtime",
  ...
  function fromSnapshot() {
    ...
    return {
      ...
      acceptEncoding: pickFirst(snap, ACCEPT_ENCODING_KEYS) || pickFirst(snap, ["acceptEncoding"]),
      source: "snapshot",
  ...
  function resolve() {
    ...
    // Unlike the others, an EMPTY value is meaningful here: GLMP_ACCEPT_ENCODING=""
    // has to mean "omit the header entirely", so unset and empty cannot collapse
    // into the same `|| null` the other overrides use.
    const ovAcceptEncoding = process.env.GLMP_ACCEPT_ENCODING !== undefined
      ? process.env.GLMP_ACCEPT_ENCODING.trim()
      : null;
    const overridden = Boolean(ovVersion || ovUa || ovAccept || ovAcceptEncoding);
    ...
    const acceptEncoding = ovAcceptEncoding !== null
      ? ovAcceptEncoding
      : (discovered?.acceptEncoding || DEFAULT_ACCEPT_ENCODING);
  ...
  function capture(identity) {
    ...
    try {
      writeJsonAtomic(snapshotPath(), {
        ...
        acceptEncoding: identity.acceptEncoding || null,
  ...
  function freshness() {
    ...
    return {
      ...
      acceptEncoding: identity.acceptEncoding,
