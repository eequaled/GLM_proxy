...
// ============================================================================
// Token lifetime — the one row an operator cannot infer for themselves
// ============================================================================

// The proxy authenticates with a *captured* file, not a client that refreshes
// itself: AutoClaw rewrites `request-headers.json` hourly, and if the app stops
// running, this proxy keeps sending a token that has quietly died. Upstream
// then answers 401, which reads like an account problem when it is only a stale
// file — so `--doctor` says it in advance, and says what to do about it, rather
// than leaving the operator to decode an epoch by hand.
//
// Pure (claims in, a line out), never throws, and a token with no `exp` admits
// it cannot be judged instead of inventing a window.
export function describeTokenWindow(info, nowMs = Date.now()) {
  const lifetimeSeconds = Number(info?.lifetimeSeconds);
  const lifetime = Number.isFinite(lifetimeSeconds) && lifetimeSeconds > 0
    ? `${Math.round(lifetimeSeconds / 3600)}h token`
    : "token";
  ...
  if (!Number.isFinite(exp) || exp <= 0) {
    return {
      ...
      remainingMs: null,
      lifetimeHours: null,
      text: `${lifetime} expiry unknown — the claims carried no exp, so freshness cannot be judged`,
    };
  }

  const remainingMs = exp * 1000 - nowMs;
  const lifetimeHours = Number.isFinite(lifetimeSeconds) && lifetimeSeconds > 0
    ? Math.round(lifetimeSeconds / 3600)
    : null;

  // An hour is the useful horizon: under it the operator still has time to
  // re-capture while the app is logged in, which is the only thing that fixes it.
  if (remainingMs <= 0) {
    return {
      warn: true,
      remainingMs,
      lifetimeHours,
      text: `${lifetime} expired — re-capture it while the app is logged in`,
    };
  }
  ...
  if (remainingMs < 3_600_000) {
    ...
    return {
      ...
      remainingMs,
      lifetimeHours,
      text: `${lifetime} — expires in ${mins}m; re-capture it while the app is logged in`,
    };
  }
  ...
  return {
    ...
    remainingMs,
    lifetimeHours,
    text: `${lifetime} — expires in ${(remainingMs / 3_600_000).toFixed(1)}h`,
  };
}
